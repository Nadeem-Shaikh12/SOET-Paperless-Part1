...
export default function SubjectsClassesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (tab === 'subjects') {
        ...
        if (editItem) await apiClient(`/subjects/${editItem.id}`, { method: 'PUT', body: JSON.stringify(payload) });else
        await apiClient('/subjects', { method: 'POST', body: JSON.stringify(payload) });
      } else if (tab === 'classes') {
        const payload = { ...formData, deptId: Number(formData.deptId), termId: Number(formData.termId) };
        if (editItem) await apiClient(`/classes/${editItem.id}`, { method: 'PUT', body: JSON.stringify(payload) });else
        await apiClient('/classes', { method: 'POST', body: JSON.stringify(payload) });
      } else if (tab === 'batches') {
  ...
  const handleDelete = async (item) => {
    ...
    try {
      let endpoint = '';
      if (tab === 'subjects') endpoint = `/subjects/${item.id}`;else
      if (tab === 'classes') endpoint = `/classes/${item.id}`;else
      if (tab === 'batches') endpoint = `/classes/batches/${item.id}`;else
      if (tab === 'strength') endpoint = `/student-strength/${item.id}`;
