...
import { Calendar, Search, MapPin, Building, Pencil, Save, X, Printer, FileSpreadsheet } from 'lucide-react';
...
export function FacultyScheduleReport() {
  ...
  const filteredData = useMemo(() => {
    return activeData.filter((faculty) => {
      const matchFaculty = selectedFaculty ? faculty.facultyName.toLowerCase().includes(selectedFaculty.toLowerCase()) : true;
  ...
  const getStatusColor = (activity) => {
    ...
    if (lower.includes('meeting') || lower.includes('admin')) return 'bg-amber-50 border-amber-200 text-amber-800';
    if (lower.includes('lab') || lower.includes('practical')) return 'bg-blue-50 border-blue-200 text-blue-800';
  ...
  const handleDownloadExcel = () => {
    // Construct the 2D array for Excel
    ...
    filteredData.forEach(faculty => {
      ...
      timeSlots.forEach(time => {
        const cellData = dailySchedule.find(entry => entry.time === time);
        if (cellData) {
          row.push(`${cellData.activity} ${cellData.location ? '(' + cellData.location + ')' : ''}`);
        } else {
          row.push("Unscheduled");
        }
      });
  ...
  const updateCell = useCallback((facultyName, time, field, value) => {
    setEditData((prev) => {
      const updated = [...prev];
      ...
      if (!faculty.schedule[currentDay]) {
        faculty.schedule[currentDay] = [];
      }
      
      let entryIndex = faculty.schedule[currentDay].findIndex(e => e.time === time);
      if (entryIndex === -1) {
        faculty.schedule[currentDay].push({ time, activity: '', location: '', floor: '' });
        entryIndex = faculty.schedule[currentDay].length - 1;
      }

      faculty.schedule[currentDay][entryIndex][field] = value;
      return updated;
    });
  }, [currentDay]);
    ...
    <div className="space-y-6 print:space-y-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--color-slate)] mt-1">Day-wise timeline view of faculty activities and locations.</p>
        </div>
        
        <div className="flex items-center gap-2">
          {editing ? (
            <>
              <button onClick={cancelEditing} className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border border-[var(--border)] text-[var(--color-graphite)] hover:bg-[var(--color-fog)] transition-all">
                <X className="w-4 h-4" /> Cancel
              </button>
              <button onClick={handleSave} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg bg-[var(--color-carbon)] text-white hover:bg-[var(--color-graphite)] transition-all disabled:opacity-50">
                <Save className="w-4 h-4" /> {saving ? 'Saving...' : 'Save'}
              </button>
            </>
          ) : (
            <>
              <button onClick={handleDownloadExcel} className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border border-[var(--border)] text-[var(--color-graphite)] hover:bg-[var(--color-fog)] transition-all">
                <FileSpreadsheet className="w-4 h-4" /> Download Excel
              </button>
              <button onClick={handlePrint} className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border border-[var(--border)] text-[var(--color-graphite)] hover:bg-[var(--color-fog)] transition-all">
                <Printer className="w-4 h-4" /> Print
              </button>
              <button onClick={() => setEditing(true)} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg bg-[var(--color-carbon)] text-white hover:bg-[var(--color-graphite)] transition-all">
              ...
              <table className="w-full text-xs border-collapse schedule-table">
                <thead>
                  <tr className="bg-[var(--color-fog)]">
                    ...
                    {timeSlots.map(time => (
                      <th key={time} className="sch-th min-w-[140px]">
                        {time}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {filteredData.map((faculty, idx) => {
                    ...
                    return (
                      <tr key={faculty.facultyName} className="hover:bg-[var(--color-fog)]/50 transition-colors group">
                        ...
                        {timeSlots.map(time => {
                          if (time === '12:00-12:45') {
                            return (
                              <td key={`${faculty.facultyName}-${time}`} className="sch-td text-center align-middle bg-[var(--color-fog)]/50">
                          ...
                          return (
                            <td key={`${faculty.facultyName}-${time}`} className="sch-td text-center align-middle">
                              {editing ? (
                                ...
                                cellData && cellData.activity ? (
                                  ...
                                  <span className="text-[10px] text-[var(--color-carbon)] opacity-50 uppercase tracking-wider">Unscheduled</span>
                                )
                              )}
                            </td>
                          );
      ...
      {/* Component-scoped styles for print and tables */}
      <style>{`
        .sch-th {
          padding: 8px 12px;
          text-align: center;
        ...
        @media print {
          ...
          /* Adjust table scaling for print */
          .schedule-table {
            zoom: 0.8;
          }
